...
class KeyboardWindow :  public TouchWindow
{
	...
	char _editBuffer[15];
	int  _editPosition;
	const static int _buttonSize=35;
	const static int _buttonDistance=5;
	const static int _textOffset=7;
public:
	KeyboardWindow(int left,int top,int width,int height):TouchWindow(F(""),left,top,width,height)
	{
		_targetTextBox=NULL;
		this->SetBorder(Color::CornflowerBlue);
		//_digidWindows=new TextBoxNumber *[10];
		int x=0;
		int y=0;
		_editField=new TextBoxString<char>(0,y,7*(_buttonSize+_buttonDistance)-_buttonDistance,_buttonSize,"",Color::DarkBlue);
		...
		for(int i=0;i<10;i++)
		{
			...
			if(i==5)
				y+=_buttonSize+_buttonDistance;
			_digidWindows[i]=new TextBoxNumber(x,y,_buttonSize,_buttonSize,0,Color::DarkBlue,true);
